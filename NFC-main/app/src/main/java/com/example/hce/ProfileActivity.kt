...
class ProfileActivity : AppCompatActivity() {
    ...
    private lateinit var keyRefresh:Button
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        keyRefresh = findViewById(R.id.keyRefresh)
        ...
        keyRefresh.setOnClickListener{
            val uid = sharedPref.getString("employee_id", "")

            // Check if employee ID is available from SharedPreferences
            if (uid.isNullOrEmpty()) {
                // Handle case where employee ID is not available
                Log.e("UpdateKeyTask", "Employee ID not found in SharedPreferences")
                return@setOnClickListener
            }

            // Convert employee ID to integer (assuming it's stored as a string)

            // Create and execute the AsyncTask to update the key
            val updateKeyTask = UpdateKeyTask(uid)
            updateKeyTask.execute()
        }
    }
